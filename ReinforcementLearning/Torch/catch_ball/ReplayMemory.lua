...
function ReplayMemory(maxMemory, gridSize, numAction, discount)
  local memory = {}
  
  -- 새로운 경험 (s,a,r,s') 기억
  function memory.remember(currentState, action, reward, nextState, gameOver)
    local memoryInput = {
  ...
  function memory.getBatch(batch_size, qlearningAgent)
    ...
    local inputs = torch.Tensor(chosenBatchSize, gridSize, gridSize):zero()
    local targets = torch.Tensor(chosenBatchSize, numAction):zero()
