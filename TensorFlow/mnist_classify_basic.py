...
# 히든 레이어
W1 = tf.Variable(tf.truncated_normal([image_size*image_size, 128],
...
# 출력 레이어
W3 = tf.Variable(tf.truncated_normal([64, class_num],
...
for step in range(step_num):
  if (batch_num+1) * batch_size <= train_img.shape[0]:
    batch_xs = train_img[batch_num*batch_size:(batch_num+1)*batch_size]
    batch_ys = train_label[batch_num*batch_size:(batch_num+1)*batch_size]
...
while (batch_num+1) * batch_size <= val_img.shape[0]:
  ...
  print(step_eval_accuracy)
  eval_accuracy = eval_accuracy + step_eval_accuracy
  batch_num = batch_num + 1

print(eval_accuracy / batch_num * 100)